...
func (m Model) headersView() string {
	...
	for _, col := range m.cols {
		style := lipgloss.NewStyle().Width(col.Width).MaxWidth(col.Width).Inline(true).AlignHorizontal(lipgloss.Center)
...
func (m *Model) renderRow(rowID int) string {
	...
	for i, value := range m.rows[rowID] {
		...
		stringToRender := style.Render(runewidth.Truncate(value, m.cols[i].Width, "…"))
		var renderedCell string
		if rowID == m.cursor {
			renderedCell = m.styles.Selected.Render(stringToRender)
		} else {
			renderedCell = m.styles.Cell.Render(stringToRender)
		}
